...
public class EnrollmentManager {
    ...
    public static List<Course> getAvailableCoursesForStudent(String studentUser) {
        ...
        for (String[] courseRow : CourseManager.getCourses()) {
            try {
                if (courseRow.length > 0) {
                    Course course = CourseManager.findCourse(Long.parseLong(courseRow[0]));
                    if (course != null) {
                        // Verificar si el estudiante ya está inscrito en este curso
                        boolean alreadyEnrolled = false;
                        for (String[] enrollment : enrollments) {
                            if (enrollment.length > 1 && enrollment[0].equals(studentUser)) {
                                Group group = GroupManager.findGroup(enrollment[1]);
                                if (group != null && group.getRepresents().getCode() == course.getCode()) {
                        ...
                        // Si no está inscrito, añadir a disponibles
                        if (!alreadyEnrolled) {
